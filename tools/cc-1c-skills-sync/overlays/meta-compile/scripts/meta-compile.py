...
def write_utf8_bom(path, content):
    if _loadconfig is not None:
        content = _loadconfig.normalize_metadata_xml_text(content)
...
def emit_type_content(indent, type_str):
    ...
    if _loadconfig is not None:
        type_str = _loadconfig.normalize_number_dsl(type_str)
    else:
        m0 = re.match(r'^Number\((\d+)\)$', type_str)
        if m0:
            type_str = f'Number({m0.group(1)},0)'
    # Boolean
    if type_str == 'Boolean':
    ...
    # Number(D) — kit-local: один параметр
    ...
    if m:
        # kit-local: cfg: для hierarchical conf/
...
def emit_standard_attribute(indent, attr_name, *, tabular_line_number=False):
    ...
    if tabular_line_number:
        X(f'{indent}\t<xr:TypeReductionMode>TransformValues</xr:TypeReductionMode>')
...
def emit_tabular_standard_attributes(indent):
    X(f'{indent}<StandardAttributes>')
    emit_standard_attribute(f'{indent}\t', 'LineNumber', tabular_line_number=True)
    X(f'{indent}</StandardAttributes>')
...
def emit_tabular_section(indent, ts_name, columns, object_type, object_name):
    ...
    if object_type == 'Catalog':
        X(f'{indent}\t\t<Use>ForItem</Use>')
    # kit-local: LineNumberLength для catalog/document
    if object_type in ('Catalog', 'Document'):
        X(f'{indent}\t\t<LineNumberLength>5</LineNumberLength>')
    X(f'{indent}\t</Properties>')
