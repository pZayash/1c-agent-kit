...
Формат JSON для описания ролей 1С, используемый скиллом `role-compile`
(`.cursor/skills/` потребителя).
...
Подробные таблицы прав для каждого типа объекта — в скилле `role-compile`
потребителя (`dsl-reference.md`, если есть).
...
DSL принимает русские имена типов и прав, транслируя их в английские. Полные таблицы соответствий — в скилле `role-compile` потребителя (`dsl-reference.md`, если есть).
