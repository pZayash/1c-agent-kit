...
Полное описание XML-формата ролей в выгрузке конфигурации. Версии формата: 2.17 (платформа 8.3.20–8.3.24), 2.20 (8.3.27+), 2.21 (8.5.1+; у потребителя — ключ `PLATFORM_VERSION` в `.env.example`). Структура идентична, отличается только атрибут `version`.
...
| 8.5.1+ | 2.21 | 2.21 | Актуальная платформа потребителя (`.env.example`) |
