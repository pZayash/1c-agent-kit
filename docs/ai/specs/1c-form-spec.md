...
## 17. Особенности форм в расширениях конфигурации (cfe)

### Код языка `v8:lang`

В расширениях конфигурации для мультиязычных значений (`<Title>`, `<ToolTip>`, `<Synonym>`) используется **код языка с суффиксом**, а не базовый код. Если в основной конфигурации язык — `ru`, то в расширении он будет `ru1` (или `ru2`, `ru3` — в зависимости от порядка расширения).
...
Это касается ВСЕХ мультиязычных элементов внутри расширения: форм, команд, реквизитов, синонимов объектов.

### Пространства ID

В Form.xml расширения действуют те же правила, что и в основной конфигурации:
- **Визуальные элементы** (ChildItems) — общее пространство ID
- **Attributes** — отдельное пространство ID
- **Commands** — отдельное пространство ID

Т.е. `<Command id="1">` НЕ конфликтует с `<Pages id="1">` или `<Attribute id="1">`.

---

## 18. Элементы, не встреченные в конфигурации
