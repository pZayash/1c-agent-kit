...
Компактный JSON-формат для описания макетов табличных документов 1С (SpreadsheetDocument). Скиллы `mxl-compile` (JSON → XML) и `mxl-decompile` (XML → JSON) — в `.cursor/skills/` потребителя.
