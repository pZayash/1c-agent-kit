...
Навыки генерируют XML с `version="2.17"`. Сборка EPF через `1cv8.exe` платформы
потребителя (`PLATFORM_BUILD` в `.env.example`) проходит успешно — платформа
принимает файлы с более старым номером версии без ошибок. Повышать `version`
до `"2.21"` не требуется.
