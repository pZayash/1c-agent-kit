...
**Путь к 1cv8.exe** — `DESIGNER_PATH` в `.env.example` потребителя (каталог `C:\Program Files\1cv8\{PLATFORM_BUILD}\bin\`).
...
| Переменная | Описание |
|-----------|----------|
| `V8_PATH` | Каталог `bin` платформы 1С — родитель `DESIGNER_PATH` из `.env.example` потребителя |
