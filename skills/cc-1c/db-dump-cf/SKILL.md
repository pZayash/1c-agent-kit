...
```bash
bash tools/sandbox/run.sh python ".cursor/skills/db-dump-cf/scripts/db-dump-cf.py" <параметры>
```
...
bash tools/sandbox/run.sh python ".cursor/skills/db-dump-cf/scripts/db-dump-cf.py" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -OutputFile "C:\backup\config.cf"

# Серверная база
bash tools/sandbox/run.sh python ".cursor/skills/db-dump-cf/scripts/db-dump-cf.py" -InfoBaseServer "srv01" -InfoBaseRef "MyApp_Dev" -UserName "Admin" -Password "secret" -OutputFile "config.cf"

# Выгрузка расширения
bash tools/sandbox/run.sh python ".cursor/skills/db-dump-cf/scripts/db-dump-cf.py" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -OutputFile "ext.cfe" -Extension "МоёРасширение"
```
