...
```bash
bash tools/sandbox/run.sh python ".cursor/skills/epf-dump/scripts/epf-dump.py" <параметры>
```
...
bash tools/sandbox/run.sh python ".cursor/skills/epf-dump/scripts/epf-dump.py" -InfoBasePath "C:\Bases\MyDB" -InputFile "build/МояОбработка.epf" -OutputDir "src"

# Серверная база
bash tools/sandbox/run.sh python ".cursor/skills/epf-dump/scripts/epf-dump.py" -InfoBaseServer "srv01" -InfoBaseRef "MyDB" -UserName "Admin" -Password "secret" -InputFile "build/МояОбработка.epf" -OutputDir "src"
```
