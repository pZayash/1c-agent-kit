...
```bash
bash tools/sandbox/run.sh python ".cursor/skills/mxl-info/scripts/mxl-info.py" -TemplatePath "<путь>"
```

Или по имени обработки/макета:
```bash
bash tools/sandbox/run.sh python ".cursor/skills/mxl-info/scripts/mxl-info.py" -ProcessorName "<Имя>" -TemplateName "<Макет>" [-SrcDir "<каталог>"]
```
