...
```bash
# Из файла
bash tools/sandbox/run.sh python ".cursor/skills/skd-compile/scripts/skd-compile.py" -DefinitionFile "<json>" -OutputPath "<Template.xml>"

# Из строки (без промежуточного файла)
bash tools/sandbox/run.sh python ".cursor/skills/skd-compile/scripts/skd-compile.py" -Value '<json-string>' -OutputPath "<Template.xml>"
```
