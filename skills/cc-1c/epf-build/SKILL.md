...
```bash
bash tools/sandbox/run.sh python ".cursor/skills/epf-build/scripts/epf-build.py" <параметры>
```
...
bash tools/sandbox/run.sh python ".cursor/skills/epf-build/scripts/epf-build.py" -InfoBasePath "C:\Bases\MyDB" -SourceFile "src/МояОбработка.xml" -OutputFile "build/МояОбработка.epf"

# Серверная база
bash tools/sandbox/run.sh python ".cursor/skills/epf-build/scripts/epf-build.py" -InfoBaseServer "srv01" -InfoBaseRef "MyDB" -UserName "Admin" -Password "secret" -SourceFile "src/МояОбработка.xml" -OutputFile "build/МояОбработка.epf"
```
