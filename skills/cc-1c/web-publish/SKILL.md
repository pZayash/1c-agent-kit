...
```bash
bash tools/sandbox/run.sh python ".cursor/skills/web-publish/scripts/web-publish.py" <параметры>
```
...
```bash
# Файловая база
bash tools/sandbox/run.sh python ".cursor/skills/web-publish/scripts/web-publish.py" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin"

# С явным именем публикации и портом
bash tools/sandbox/run.sh python ".cursor/skills/web-publish/scripts/web-publish.py" -InfoBasePath "C:\Bases\MyDB" -AppName "mydb" -Port 9090

# Серверная база
bash tools/sandbox/run.sh python ".cursor/skills/web-publish/scripts/web-publish.py" -InfoBaseServer "srv01" -InfoBaseRef "MyDB" -UserName "Admin" -Password "secret"

# Ручной режим (только инструкция)
bash tools/sandbox/run.sh python ".cursor/skills/web-publish/scripts/web-publish.py" -InfoBasePath "C:\Bases\MyDB" -Manual
```
