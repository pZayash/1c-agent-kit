...
# kit-local: локальные правки LoadConfigFromFiles — см. docs/ai/metadata-xml-load-pitfalls.md
...
# kit-local: общий модуль lint/normalize (tools/metadata-xml-lint/)
def _load_loadconfig_module():
    here = Path(__file__).resolve()
    for parent in here.parents:
        mod_path = parent / "tools" / "metadata-xml-lint" / "loadconfig_xml.py"
        if mod_path.is_file():
            spec = importlib.util.spec_from_file_location("loadconfig_xml", mod_path)
...
_loadconfig = _load_loadconfig_module()
...
def resolve_type_str(type_str):
    ...
    if m:
        ...
        if resolved:
            # kit-local: Number(3) → Number(3,0)
            if resolved == "Number" and "," not in params:
                return f"Number({params.strip()},0)"
...
def normalize_type_str_for_xml(type_str):
    """kit-local: Number(D) без дробной части."""
    if _loadconfig is not None:
        return _loadconfig.normalize_number_dsl(type_str)
    m = re.match(r"^Number\((\d+)\)$", type_str)
    if m:
        return f"Number({m.group(1)},0)"
    return type_str
...
def build_type_content_xml(indent, type_str):
    ...
    type_str = normalize_type_str_for_xml(type_str)
    lines = []
    ...
    if m:
        # kit-local: cfg: для hierarchical conf/ (LoadConfigFromFiles)
...
def build_tabular_section_fragment(ts_def, indent):
    ...
    # kit-local: обязательно для LoadConfigFromFiles (эталон УНФ)
    lines.append(f"{indent}\t\t\t\t<xr:TypeReductionMode>TransformValues</xr:TypeReductionMode>")
    ...
    # kit-local: LineNumberLength для ТЧ catalog/document
    if obj_type in ("Catalog", "Document"):
        lines.append(f"{indent}\t\t<LineNumberLength>5</LineNumberLength>")
...
def save_xml(tree, path):
    ...
    # kit-local: &#13;, d5p1→cfg, LF (см. tools/metadata-xml-lint/loadconfig_xml.py)
    if _loadconfig is not None:
        xml_bytes = _loadconfig.normalize_metadata_xml_bytes(xml_bytes)
    else:
...
def print_loadconfig_scan_warnings(path):
    """kit-local: предупреждения после save."""
    if _loadconfig is None:
        return
    issues = _loadconfig.scan_issues_file(Path(path))
    for issue in issues:
        warn(f"LoadConfigFromFiles: {issue}")
...
def main():
    ...
    print_loadconfig_scan_warnings(resolved_path)
