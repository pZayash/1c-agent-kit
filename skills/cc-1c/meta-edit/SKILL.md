...
**kit-local:** `scripts/meta-edit.py` — локальные правки (`kit-local:`) для
LoadConfigFromFiles; скилл в `LOCAL_SKILLS` sync.py.
...
```bash
bash tools/sandbox/run.sh python ".cursor/skills/meta-edit/scripts/meta-edit.py" -ObjectPath "<path>" -Operation <op> -Value "<val>"
```

### JSON mode (сложные/комбинированные)

```bash
bash tools/sandbox/run.sh python ".cursor/skills/meta-edit/scripts/meta-edit.py" -DefinitionFile "<json>" -ObjectPath "<path>"
```
...
**После правок метаданных в `conf/`** (обязательно перед load):

1. В DSL числа — только `Number(D,F)` или `Number(D,F,nonneg)`, не `Number(D)`.
2. Проверить антипаттерны XDTO (`rg` по `d5p1:`, `Number(N)` в Type, `&#13;`).
3. `./load-changed-files.sh -U` — ground truth для hierarchical dump.

Канон: [metadata-xml-load-pitfalls.md](../../../docs/ai/metadata-xml-load-pitfalls.md).
