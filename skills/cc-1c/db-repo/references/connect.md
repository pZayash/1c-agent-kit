...
```bash
bash tools/sandbox/run.sh python ".cursor/skills/db-repo/scripts/db-repo.py" -Command connect -InfoBasePath "C:\Bases\MyDB" -RepositoryPath "\\srv01\repo\MyApp" -RepositoryUser "Ivanov" -RepositoryPassword "…"
```
...
```bash
bash tools/sandbox/run.sh python ".cursor/skills/db-repo/scripts/db-repo.py" -Command disconnect -InfoBasePath "C:\Bases\MyDB"
```
