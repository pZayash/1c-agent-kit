...
```bash
bash tools/sandbox/run.sh python ".cursor/skills/db-repo/scripts/db-repo.py" -Command set-label -InfoBasePath "C:\Bases\MyDB" -Label "Релиз 1.2" -Version 120 -Comment "Передано в тест"
```
...
```bash
bash tools/sandbox/run.sh python ".cursor/skills/db-repo/scripts/db-repo.py" -Command optimize -InfoBasePath "C:\Bases\MyDB"
```
...
```bash
bash tools/sandbox/run.sh python ".cursor/skills/db-repo/scripts/db-repo.py" -Command clear-cache -InfoBasePath "C:\Bases\MyDB" -CacheScope local
```
