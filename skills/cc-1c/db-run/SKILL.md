...
```bash
bash tools/sandbox/run.sh python ".cursor/skills/db-run/scripts/db-run.py" <параметры>
```
...
```bash
# Простой запуск
bash tools/sandbox/run.sh python ".cursor/skills/db-run/scripts/db-run.py" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin"

# Запуск с обработкой
bash tools/sandbox/run.sh python ".cursor/skills/db-run/scripts/db-run.py" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -Execute "C:\epf\МояОбработка.epf"

# Открыть по навигационной ссылке
bash tools/sandbox/run.sh python ".cursor/skills/db-run/scripts/db-run.py" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -URL "e1cib/data/Справочник.Номенклатура"

# Серверная база с параметром запуска
bash tools/sandbox/run.sh python ".cursor/skills/db-run/scripts/db-run.py" -InfoBaseServer "srv01" -InfoBaseRef "MyDB" -UserName "Admin" -Password "secret" -CParam "ЗапуститьОбновление"
```
