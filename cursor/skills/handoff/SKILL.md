...
Канон — локальный каталог `handoffs/` (вне git, в `.gitignore`).

1. Без явного пути — `handoffs/<YYYY-MM-DD>-<slug>.md`: дата локальная, `<slug>` —
   короткий kebab-топик из фокуса следующей сессии.
2. Путь из аргумента (`.md` или существующая папка) — писать туда.
3. **Перед записью** Read целевого файла. Нет файла — норма.
4. Сообщить путь и объём (строки).
...
1. Сообщить путь файла снимка и объём (строки).
